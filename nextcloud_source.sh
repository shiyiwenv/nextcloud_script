...
list(){
    ...
    [ $? -gt 0 ] && echo '[!] Curl Error listing files' && exit
...
download(){
    ...
    if [ $(exists $pname) == 1 ];then
        h1 "[+] Downloading $fname..."
        curl --progress-bar -o "$fname" $extra_opts $options -X GET $dav_url/$pname
        [ $? -gt 0 ] && echo "[!] Error downloading the file" || success "[+] Done"
    else
...
upload(){
    ...
    if [ -f "$1" ];then
        ...
        [ $? -eq 0 ] && success "[+] Done" || error "[!] Error uploading the file. Verify target directory exists"
    else
...
[[ -z `echo $1|cut -c 3` ]] && error $"Error: You should nextcloud -uyouname options ,Not nextcloud -u youname options" && exit
[[ -z `echo $2` ]] && error $"Error: Options null" && usage && exit
#if [[ -z `echo $2|cut -c 3-` ]];then
	read -p "password:" pass
conf=(
username=`echo $1|cut -c 3-`
password=$pass
host=http://xxx.com
trust_cert=no
trail=
...
#	while [[ $# > 1 ]];do
        opt="$2"
        case $opt in
            -d|--download)  download "$3"
            shift
            ;;
            -l|--list)      list "$3"
            shift
            ;;
            -U|--upload)    upload "$3" "$4"; exit
            shift
            ;;
            -D|--delete)    delete "$3"
            shift
            ...
            shift
            ;;
            *)              echo -en "Unknown option or option is null '$opt'\n\n" && usage && exit
        esac
        shift
 #       done
fi
...
usage(){
    ...
    echo "Options:"
    echo "   -u/--name			     Exp:-uadmin"
#    echo "   -p/--password		     Exp:-p123456" 
...
setenv(){
	for l in ${conf[@]}; do [ "${l::1}" != "#" ] && export $l; done
